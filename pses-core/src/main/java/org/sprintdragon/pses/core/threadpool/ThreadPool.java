package org.sprintdragon.pses.core.threadpool;
...
import org.sprintdragon.pses.core.common.settings.SettingsException;
import org.sprintdragon.pses.core.common.unit.SizeValue;
import org.sprintdragon.pses.core.common.unit.TimeValue;
import org.sprintdragon.pses.core.common.util.concurrent.EsAbortPolicy;
import org.sprintdragon.pses.core.common.util.concurrent.EsExecutors;
import org.sprintdragon.pses.core.common.util.concurrent.EsThreadPoolExecutor;
import org.sprintdragon.pses.core.common.util.concurrent.XRejectedExecutionHandler;

import java.util.*;
import java.util.concurrent.*;

import static org.sprintdragon.pses.core.common.settings.Settings.settingsBuilder;
import static org.sprintdragon.pses.core.common.unit.SizeValue.parseSizeValue;
import static org.sprintdragon.pses.core.common.unit.TimeValue.timeValueMinutes;
...
//    }
    ...
    private ExecutorHolder rebuild(String name, ExecutorHolder previousExecutorHolder, @Nullable Settings settings, Settings defaultSettings) {
        ...
        if (type == null) {
            type = previousInfo != null ? previousInfo.getThreadPoolType().getType() : defaultSettings.get("type");
        }
        ...
        if (ThreadPoolType.DIRECT == threadPoolType) {
            if (previousExecutorHolder != null) {
                log.debug("updating thread_pool [{}], type [{}]", name, type);
            } else {
                log.debug("creating thread_pool [{}], type [{}]", name, type);
            }
            ...
            if (previousExecutorHolder != null) {
                if (ThreadPoolType.CACHED == previousInfo.getThreadPoolType()) {
                    ...
                    if (!previousInfo.getKeepAlive().equals(updatedKeepAlive)) {
                        log.debug("updating thread_pool [{}], type [{}], keep_alive [{}]", name, type, updatedKeepAlive);
            ...
            if (previousExecutorHolder != null) {
                log.debug("updating thread_pool [{}], type [{}], keep_alive [{}]", name, type, keepAlive);
            } else {
                log.debug("creating thread_pool [{}], type [{}], keep_alive [{}]", name, type, keepAlive);
            }
                ...
                if (ThreadPoolType.FIXED == previousInfo.getThreadPoolType()) {
                    ...
                    if (Objects.equals(previousInfo.getQueueSize(), updatedQueueSize)) {
                        ...
                        if (previousInfo.getMax() != updatedSize) {
                            log.debug("updating thread_pool [{}], type [{}], size [{}], queue_size [{}]", name, type, updatedSize, updatedQueueSize);
            ...
            log.debug("creating thread_pool [{}], type [{}], size [{}], queue_size [{}]", name, type, size, queueSize);
            ...
            if (previousExecutorHolder != null) {
                log.debug("updating thread_pool [{}], type [{}], min [{}], size [{}], keep_alive [{}]", name, type, min, size, keepAlive);
            } else {
                log.debug("creating thread_pool [{}], type [{}], min [{}], size [{}], keep_alive [{}]", name, type, min, size, keepAlive);
            }
    ...
    class LoggingRunnable implements Runnable {
        ...
        public void run() {
            try {
                ...
                log.warn("failed to run {}", t, runnable.toString());
                throw t;
            }
        }
